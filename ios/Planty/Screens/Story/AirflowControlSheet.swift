...
struct AirflowControlSheet: View {
    ...
    private func airflowSection(_ actuator: Actuator) -> some View {
        if let lease = activeLease, lease.isActive {
            Section {
                ...
                Button("Stop airflow", role: .destructive) {
                    Task { await stop(actuator) }
                }
            ...
            Section {
                Label(actuator.name, systemImage: "fan.fill")
                    .font(.headline)
    ...
    private func sharedImpact(for actuator: Actuator, alreadyRecorded: Bool) -> String {
        ...
        guard !otherIDs.isEmpty else {
            return "Planty \(verb) one airflow entry on \(plant.commonName). The fan stops automatically at the selected time."
        }
        ...
        return "This fan is shared. Planty \(verb) one airflow entry for each assigned plant: \(plant.commonName) and \(others). It stops automatically at the selected time."
    }
