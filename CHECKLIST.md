...
- [x] **The cold watch had never once worked against real Home Assistant.** Two bugs stacked, and only deploying found either. HA returns `{changed_states, service_response: {<entity>: {forecast}}}`; the client decoded the *top level* as the entity map, so it hit `changed_states`, an array, and failed to unmarshal. Both fakes had been written to my own flattened shape, so every test agreed with the bug. They now use the shape HA actually returns
- [x] **`weather.nws_home` cannot produce a daily forecast.** It reports `supported_features: 6`, hourly and twice_daily, and the cold watch asks for daily, so Home Assistant answers 500. `weather.forecast_home` reports 3, which includes daily. The code's hardcoded default is still the entity that cannot do it, which is worth fixing: better would be to ask the entity what it supports and take the best available
