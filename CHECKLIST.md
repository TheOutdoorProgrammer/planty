...
- [x] **The watering loop was scheduled nowhere.** `water` was implemented, wired into main, covered by tests and listed in the usage text, and no CronJob ran it, so the LetPot line would never once have been watered. It runs hourly now: hourly clears the 45 minute settle window so a run never judges soil still drinking, and waking hours only, because a mixed line asks somebody to go move a dripper and a 3am ask is one you learn to ignore
