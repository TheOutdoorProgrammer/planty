...
func (s *Server) listIncidents(w http.ResponseWriter, r *http.Request) {
	...
	if raw := strings.TrimSpace(r.URL.Query().Get("status")); raw != "" {
		...
		case plant.IncidentOpen, plant.IncidentAcknowledged, plant.IncidentResolved:
			status = value
		default:
			s.fail(w, r, http.StatusBadRequest, errors.New("status must be open, acknowledged, or resolved"))
			return
		}
	}
...
func requestedIncidentID(w http.ResponseWriter, r *http.Request, s *Server) (uuid.UUID, bool) {
	...
	if err != nil {
		s.fail(w, r, http.StatusBadRequest, fmt.Errorf("incident id must be a UUID: %w", err))
